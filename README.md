...
The default `JSON` confuration file: `unishellect.json` will be created in the corresponding `%AppData%\UniSHellect\` or `.config/UniShellect/` directory in `Windows` and `Linux` respectively on first run if no `-c` option has been passed with an alternate configuration file.
